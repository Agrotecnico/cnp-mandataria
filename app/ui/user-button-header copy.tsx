...
export default function UserButtonHeader( { user }: { user: User | undefined } ) {
  ...
  useEffect(() => {
    ...
    // setImgUrlSession(data)

    // const data2= sessionStorage.getItem('imgVisitor')
    // setImgVisitor(data2)
    sessionStorage.getItem('imgVisitorx') ? setImgVisitor(`${sessionStorage.getItem('imgVisitorx')}`) : sessionStorage.getItem('imgVisitor') && setImgVisitor(`${sessionStorage.getItem('imgVisitor')}`) /* : null */

    // const data3= sessionStorage.getItem('nameVisitor')
    // setNombre(data3)
    sessionStorage.getItem('imgVisitorx') && setImgVisitorx(`${sessionStorage.getItem('imgVisitorx')}`)
    // user?.name ? setNombre(user.name) : sessionStorage.getItem('nameVisitor') ? setNombre(`${sessionStorage.getItem('nameVisitor')}`) : setNombre("")
    sessionStorage.getItem('nameVisitor') && setNombre(`${sessionStorage.getItem('nameVisitor')}`)

    // sessionStorage.getItem('imgUrlSession') ? setImageUrl(`${sessionStorage.getItem('imgUrlSession')}`) : sessionStorage.getItem('imgVisitor') ? setImageUrl(`${sessionStorage.getItem("imgUrlSession")}`) : setImageUrl("")
    sessionStorage.getItem('imgUrlSession') && setImgUrlSession(`${sessionStorage.getItem('imgUrlSession')}`) 


  }, []);

  console.log("imgUrlSessionx: ", imgUrlSessionx)
  ...
  return (
    ...
    <div className='flex items-center justify-end gap-4 '>
        ...
        {user ? (
          <DropdownMenu>
            <DropdownMenuTrigger >
              <div className={`relative items-center justify-center `} >
                  {user.image /* || imgUserSession */ ? (
                    <Image 
                      src= { user.image /* ? user.image : imgUserSession! */ }
                  ...
                  <Image 
                      src= { imageUrl /* ? user.image : imgUserSession! */ }
                      alt="imagen de perfil"
                      className="h-10 w-10 rounded-full "
                      width={40}
                      height={40}>
            ...
            >
              ...
              <Button
                ...
                // disabled={!imgVisitorx ? false : true }
              >
          ...
          <DropdownMenu>
            ...
            >
              ...
              <Button
                variant={'ghost'}
                className="!p-0 !justify-start file:ml-auto h-auto w-full  text-[#374151] opacity-80 hover:opacity-100 active:opacity-70 disabled:opacity-40 hover:bg-[#548eff18]"
                onClick={ () => {
                  setIsModalOpen(true)
                }}
                disabled={!imgVisitorx ? false : true }
              >
                <DropdownMenuItem className='w-full px-4 py-1'>
                  <CameraIcon className="w-4 mr-2 text-[#39507f]" />
                  <p>{imgVisitor ? "Cambiar" : "Cargar"} imagen</p>
                </DropdownMenuItem>
              </Button>
      ...
      <div className={`fixed inset-0 bg-gray-600 bg-opacity-50 items-start justify-end ${isModalOpen2 ? "flex" : "hidden"}`}>
        <div className="relative w-[300px] mt-[64px] mr-10 bg-white rounded-md shadow-lg sm:mr-12 sm:mt-[76px] min-[1024px]:mr-[calc((100vw_-_928px)_/_2)]">
          <EditProfileName user={user} /* isModalOpen2={isModalOpen2} *//>
          <button
      ...
      <div className={`fixed inset-0 bg-gray-600 bg-opacity-50 items-start justify-end ${isModalOpen3 ? "flex" : "hidden"}`}>
        <div className="relative w-[300px] mt-[64px] mr-10 bg-white rounded-md shadow-lg sm:mr-12 sm:mt-[76px] min-[1024px]:mr-[calc((100vw_-_928px)_/_2)]">
          <EditProfileEmail user={user} isModalOpen3={isModalOpen3} />
          <button
