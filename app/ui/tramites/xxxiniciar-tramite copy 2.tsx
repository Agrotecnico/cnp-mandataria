  ...
  return (
    <>
      ...
      { !user ? (
            ...
            >
              ...
              <div className={`pt-2 sm:pt-4`}> 
                <form action={formActionx}>
                  <fieldset className={`mb-2 flex flex-col items-center gap-2 md:mb-4 md:flex-row md:gap-4`}>
                    <InputCnp
                  ...
                  {/* button submit */}
                  <ButtonA
                    className={`${(isPendingx || isPendingAuth) && "before:absolute before:inset-0 before:-translate-x-full before:animate-[shimmer_1s_infinite] before:bg-gradient-to-r before:from-transparent before:via-white/30 before:to-transparent"}  relative overflow-hidden  h-8 text-[13px] w-max ml-auto ${!open && "hidden"}`}
                    onClick={() => { 
          ...
          <Frente className="flex items-center gap-5 p-2 mt-2 text-small-regular sm:p-4 !bg-[#020b1d16]">
            <IconRegistro className=" w-[24px] ml-1.5 sm:ml-3" />
            <div className={`w-full font-medium text-start text-[13px] text-[#39507f] transition-[opacity] duration-300 sm:text-sm `}>
              <p>Te enviaré el presupuesto por email a<span className= "underline decoration-[#020b1d81] underline-offset-2 mx-1 "> {user.email}. </span></p>
            </div>
          </Frente>
        )
      }
      ...
      {estado?.message === "tramiteIniciado" && (
        <Frente className="!p-2 mt-2 !bg-[#d7e5d9] sm:!p-4 ">
          <div className={`w-full text-start text-sm transition-[opacity] duration-300 sm:text-[15px] `}>
            <p>Recibí la consulta, te responderé a la mayor brevedad.</p>
      ...
      <div className=" w-full flex justify-between items-center gap-4">
        <p className={`text-xs ml-2 text-[#020b1dcc] ${tramite && (images.length === documentos?.length || tramiteMd.slug === "x-Otros") && /* infos.length === lengthInformations && */ (emailSession || user )  && "opacity-0" } sm:text-[13px]`}><span className=" text-[#ff0000]">*</span> Requeridos</p>
            ...
            <form 
              ...
              <div className="group relative w-full flex justify-between">
                ...
                <div className="w-[170px] absolute bottom-8 pt-3">
                  <span className={`opacity-0 invisible text-xs text-[#020b1d] w-[188px] absolute bottom-[12px] bg-[#ffffff] pt-[3px] pb-[5px] pl-1.5 pr-3 rounded-lg duration-150 shadow-[0_20px_25px_-5px_rgb(0_0_0_/_0.2),_0_8px_10px_-6px_rgb(0_0_0_/_0.2),_0px_-5px_10px_#00000012] ${images.length === documentos?.length  /*&&  infos.length === lengthInformations &&  sessionStorage.getItem('email')*/ ? "" : "group-hover:opacity-100"} sm:text-[13px] group-hover:visible`}><span className="text-base text-[#ff0000]">* </span>Completar requeridos</span>
                </div>
