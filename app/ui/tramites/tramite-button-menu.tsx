    ...
    <div className="block min-[900px]:hidden sm:ml-0">
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild >
          <button
            className="inline-flex items-center justify-center gap-4 w-60 h-[26px] text-[14px] opacity-90 px-2 rounded-md bg-[#548eff] text-[#ffffff] duration-150 outline-none hover:opacity-100 sm:h-[28px] sm:text-[15px] sm:px-3"
          >
          ...
          >
            {/* {allTramites.map((tramite:TramiteMd) => (
              <Link
                ...
                className={clsx('group w-full h-[33px] border-x border-[#39507f19] text-sm flex items-center justify-start first:rounded-t-md last:rounded-b-md [box-shadow:_inset_0_1px_#ffffff,inset_0_-1px_#0000001e] duration-150 bg-[#edf1ff] text-[#020b1daa]  hover:bg-[#dde7fc] hover:text-[#020b1dcc] active:opacity-80',
                  {
                    'text-[#020b1dcd] bg-[#dde7fe] ':  pathname === `/iniciar-tramite/${tramite.slug}`,
                  }
                )}
                key={tramite.slug} 
              >
            ...
            {allTramites.map((tramite:TramiteMd) => (
              <Link
                  as={`/iniciar-tramite/${tramite.slug}`}
                  href="/iniciar-tramite/[slug]"
                  className={clsx('group w-full h-[33px] border-x border-[#39507f19] text-sm flex items-center justify-start first:rounded-t-md last:rounded-b-md [box-shadow:_inset_0_1px_#ffffff,inset_0_-1px_#0000001e] duration-150 bg-[#edf1ff] text-[#020b1d99]  hover:bg-[#dde7fc] hover:text-[#020b1dcc] active:opacity-80',
                  {
                    'text-[#020b1dcd] bg-[#dde7fd] ':  pathname === `/iniciar-tramite/${tramite.slug}`,
                  }
                )}
                  key={tramite.slug} 
              >
