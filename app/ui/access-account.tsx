...
import  AccountVerifyEmail  from "@/app/ui/account-verify-email";
  ...
  return (
    <main className="flex items-start justify-center ">
      <div className="relative flex w-full max-w-[472px] flex-col">
        {open && (user?.role === "memberAccount" || user?.role === "admin") ? (
          <Suspense>
              <LoginAccount user= {user} setOpen={setOpen} />
          </Suspense>
        ) : user?.role === "memberVerified" && (
          <Suspense>
              <RegisterAccount user= {user} setOpen={setOpen} />
          </Suspense>
        )/*  : user?.role === "member" && (
          <Suspense>
              <AccountVerifyEmail user= {user} setOpen={setOpen} />
          </Suspense>
        ) */
      }
