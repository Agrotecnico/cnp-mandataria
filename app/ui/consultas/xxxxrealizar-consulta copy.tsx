    ...
    <>
          ...
          <DisclosurePanel
            ...
            >
              {({
                ...
                <div className={`flex flex-col bg-[#020b1db8] rounded-lg ${!images.length ? 'gap-0' : 'gap-0.5'} ${!state && "invisible"} sm:flex-row`} >
                  <button
                    type="button"
      ...
      { !emailx && !emailxx && !session && !polo /* && !searchParams.get('verified') */ ? (
          ...
          <Frente className="p-2 mt-4 text-small-regular sm:p-4 !bg-[#d7e5d9]">
            <div className={`w-full text-start text-[13px] text-[#020b1ddd] transition-[opacity] duration-300 sm:text-sm `}>
              <p className="font-medium mb-1">{searchParams.has("verified") && "Correo electrónico verificado"} </p>
              <p>Te enviaremos la respuesta al email<span className= "underline decoration-[#020b1d81] underline-offset-2 mx-1 ">{session?.user.email} </span></p>
            </div>
          </Frente> 
        )
      }
                ...
                <ButtonA
                  ...
                  onClick={() => {
                    ...
                    wait().then(() =>{ 
                      ...
                      /* user?.role === "member" && */ /* updateUserEmailVerifiedx(`${user?.email}`, `${user?.email_verified}`) */

                      // console.log("userCliente: ", user)
                      // sessionStorage.setItem('emailxx', "xxxx") ;
                      // user?.role == "memberAccount" && setEmailxx("xxxx")
                      // user?.role == "memberAccount" && setEmailxx(`${sessionStorage.getItem('emailxx')}`)
                      ...
                      //   handleClickButtonVerification()
                      //   handleClickButtonPedido()
                      // } else if (user?.role === "memberAccount") {
                      //   handleClickButtonVerification()
                      //   handleClickButtonPedido()
                      //   return null
                      // }
