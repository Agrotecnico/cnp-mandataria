  ...
  return (
    <>
            ...
            sessionStorage.getItem('email') ? (
              <Frente className="p-2 mt-2 text-small-regular sm:p-4 !bg-[#e0e6e1]">
                <div className={`w-full text-start text-[13px] text-[#020b1ddd] transition-[opacity] duration-300 sm:text-sm `}>
                  <span className="mr-3 font-semibold text-sm sm:text-[15px]">{ name }</span> 
                  Para enviarte la respuesta se registró el e-mail 
                  {/* Por favor, revisá el correo electrónico
                  <span className= "underline decoration-[#020b1d81] underline-offset-2 mx-1 text-sm sm:text-[15px] ">{email}</span>
                  y enviá la verificación. */}
              ...
              <Frente className="p-2 mt-2 text-small-regular sm:p-4 !bg-[#e0e6e1]">
                <div className={`w-full text-start text-[13px] text-[#020b1ddd] transition-[opacity] duration-300 sm:text-sm `}>
                  <p>Para enviarte la respuesta se registró el e-mail</p>
                  <p>
                    <span className="mr-3 font-semibold text-sm sm:text-[15px]">{ name }</span> Por favor, revisá el correo electrónico <span className= "underline decoration-[#020b1d81] underline-offset-2 mx-1 text-sm sm:text-[15px] ">{email}</span> y enviá la verificación.
                  </p>
                </div>
              </Frente> 
            )
      ...
      {estado?.message === "consultaCreada" && (
        ...
        <Frente className="p-2 mt-2 text-small-regular sm:p-4 !bg-[#d7e5d9]">{/* #e0e6e1 */}
          <div className={`w-full text-start text-[13px] text-[#020b1ddd] transition-[opacity] duration-300 sm:text-sm `}>
            <p className="mb-1 font-semibold text-sm sm:text-[15px]">{ name }<span className="ml-2 font-normal text-xs sm:text-[13px] ">{/* {email} */}</span></p>
            <p>Recibimos la consulta, te responderemos a la brevedad.{/* Para enviarte la respuesta se registró el e-mail */}</p>
            <p>
          ...
          {estado?.message !== "consultaCreada" ? (
            <form onSubmit={ files.length === 0 ? uploadToServer1 : uploadToServer2 } >
              <div className="group relative w-full flex justify-end items-center">
                <span className={`opacity-0 invisible text-xs text-[#020b1d] absolute w-[170px] -bottom-[100%] bg-[#ffffff] pt-[3px] pb-[5px] pl-1.5 pr-3 rounded-lg duration-150 shadow-[0_20px_25px_-5px_rgb(0_0_0_/_0.2),_0_8px_10px_-6px_rgb(0_0_0_/_0.2),_0px_-5px_10px_#00000012] ${consulta && (emailSession || user) ? "" : "group-hover:opacity-100"} sm:text-[13px] group-hover:visible`}><span className="text-base text-[#ff0000]">* </span>Completar requeridos</span>
                <ButtonA
