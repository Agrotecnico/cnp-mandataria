...
import { createComment, StateComment, updateUserImage, StateUserImage, updateCommentAvatar, StateUpdateCommentAvatar } from '@/app/lib/actions';
...
export default function ListComment({ 
  ...
  useEffect(() => {
    user?.name && setNameVisitor(user.name)  
  ...
  const updateCommentAvatarWithId = updateCommentAvatar.bind(null, id4);
  const [estadoxxx, formActionxxx] = useActionState(updateCommentAvatarWithId, initialStatexxx);
        ...
        <form action={formAction}>
          <div className="hidden flex-col text-sm justify-center gap-3 ml-2 ">
            <input
              ...
              id="nombre"
              name="nombre"
              value= { user?.name ? user?.name : nameVisitor ? nameVisitor : nombre ? nombre : "" }
              readOnly
            />
            <input
              type="text"
            ...
            <input
              ...
              id="avatar"
              name="avatar"
              value= {imageUrl ? imageUrl : user?.image ? user.image : imgUrlSession ? imgUrlSession : "" }
              readOnly
            />
            <input
              type="text"
