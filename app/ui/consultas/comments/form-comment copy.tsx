    ...
    <>
      <div className={`${session && "hidden"}`}>
        <p className={` text-sm sm:text-[15px]  ${commentOk === true ? "block" : "hidden"} `}>
          ...
          </span>{/* podés realizar comentario */}
        </p> 
        ...
        <Frente className={`!p-2 w-full mt-2 mb-6 text-small-regular sm:!p-4 !bg-[#39507f11] ${commentOk === true ? "block" : "hidden"}`}>
          <div className={`flex items-center justify-between gap-2.5 sm:gap-5 ${open && "mb-4"}`}>
            ...
            </p>{/* "Para ampliarla comentá tu experiencia " */}

            <ButtonB
              className={`h-8 text-[13px]  w-max`}
              onClick={() => { setOpen(!open); setComment(""); setNombre("")/* ; setComme(true) */ }}
          ...
          {session ? (""
            ...
            <>
              ...
              <div className="" > 
                {/* nombre */}
                <fieldset className="grid grid-cols items-center gap-2 sm:grid-cols-2 md:flex-row md:gap-4">{/*  mb-2  sm:mb-3*/}
                  {!sessionStorage.getItem('nombre') &&
                    <InputCnp
                      className={`text-sm h-8 mb-4 !w-full placeholder:text-[#000000aa] placeholder:text-[13px]  ${name ? "hidden" : "block"}`}/* ${open === false ? "opacity-0" : "block"} */
                      id="nombre"
                      name="nombre"
                      type="text"
                      minLength={3}
