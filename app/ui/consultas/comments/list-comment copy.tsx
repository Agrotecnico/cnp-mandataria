...
export default function ListComment({ 
  ...
  }, [ /* nameVisitor, imgVisitor, commentSession, user?.name, user?.image */]);
    ...
    <>
      <div className={`relative ${!comme && "mt-[92px]"}`}>
        ...
        <div className={` ${images.length != 0 ? "absolute -top-[60px]" : "absolute -top-[60px]"} -left-[5px] w-full gap-1 flex flex-col mb-3 } ${comme === true || imgVisitor  ? "hidden" : "block" } `}>
          ...
          { !user && !nameVisitor ?
          <div className={`absolute top-[118px] right-4 z-10 `}>
        ...
        <form action={formAction}>
          <div className="hidden flex-col text-sm justify-center gap-3 ml-2 ">
            <input
              ...
              id="nombre"
              name="nombre"
              // value= { user?.name ? user?.name : nombre ? nombre : nameVisitor }
            ...
            <input
              ...
              value= {user?.email ? user?.email : "comment@gmail.com"}
              readOnly
            />
            <div>
              <button
