...
export default function ListComment({ 
  ...
  console.log("imgAvatar: ", comments[0].avatar)
                ...
                <form onSubmit={ uploadToServer1 } >
                  <div className={`flex items-center justify-end w-full gap-4 mt-4 text-sm `}>
                    <ButtonA
                      ...
                      onClick={() => {
                        // setSpin(true)
                        // estado.message === "ok" && setImages([])
                        // estado.message === "ok" && setOpen(!open);
                        // estado.message === "ok" && setComme(!comme);
                        // estado.message === "ok" && setCommentOk(!commentOk)
                ...
                <div className={`flex-shrink-0 ml-0`}>
                  {comment.image || comment.avatar ? (
                    <Image
                      ...
                      // src="https://res.cloudinary.com/dchmrl6fc/image/upload/v1756213337/keskchenj81hiurtxqpa.jpg"

                      // src={`${comment.image ? comment.image : comments[0].avatar ? JSON.parse(`${comment.avatar}`) : sessionStorage.getItem("imgVisitor") }`}
