...
export default function ListComment({ 
  ...
  const [imageUrlxxx, setImageUrlxxx] = useState("");
  ...
  const uploadToServer = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setImageUrlxxx(responseData.urls[0]);
  ...
  }, [/*successState imageUrlxx */nameVisitor]);
  ...
  console.log("commentLastNombre:", commentLast.nombre)
  ...
  return (
    <>
      ...
      <div className={`relative ${!comme && "mt-[92px]"}`}>{/* space-y-5 */}
        ...
        <div className={` ${images.length != 0 ? "absolute -top-[60px]" : "absolute -top-[60px]"} -left-[5px] w-full gap-1 flex flex-col mb-3 } ${comme === true || imgVisitor  ? "hidden" : "block" } `}>
          ...
          >
            {({
              ...
              <div className={`hidden flex-col bg-[#7b5c727d] w-full rounded-lg ${!images.length ? 'gap-0' : 'gap-0.5'} `} >
                <button
                  type="button"
                  onClick={onImageUpload}
                  {...dragProps}
        ...
        <div className='relative '>
          {comments &&
            comments.map((comment, index) => {
              ...
              // const isVisitor=  nameVisitor == comments[index].nombre ;
                  ...
                  { comment.email_id === "comment@gmail.com" ? (
                    ...
                    <div>
                      ...
                      <Image
                        ...
                        className="rounded-full object-cover ml-3 h-10 w-10"
                      />
                      ) : imageUrlxxx || imgUrlSession ? (
                        <Image
                          src={ imageUrlxxx ? imageUrlxxx : imgUrlSession! }
                          alt="Imagen de usuario"
                          width={40}
                          height={40}
                          className="rounded-full object-cover ml-3 h-10 w-10"
        ...
        <form action={formActionxx}>
          <div className="">
            <input
              ...
              // value= {imageUrl ? imageUrl : "" }
              // value= {imageUrlxxx }
              defaultValue={ imageUrlxxx ? imageUrlxxx : imgUrlSession ? imgUrlSession : "" }
              // readOnly
            />
            <div>
              <button
