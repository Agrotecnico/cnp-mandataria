...
export default /* async */ function RealizarConsulta( /* { user }: { user: User | undefined  }  */ ) {
  ...
  useEffect(() => {
    ...
    // sessionStorage.getItem('emailxx') && setEmailxx(`${sessionStorage.getItem('emailxx')}`)
  ...
  // console.log("consulta: ", polo)
  // console.log("emailx: ", emailx)
  // console.log("emailxx: ", emailxx)
  // console.log("name: ", name)
  console.log("email: ", email)

  console.log("paramVerified: ", searchParams.get('verified'))
  console.log("paramName: ", searchParams.get('name'))
  console.log("paramEmail: ", searchParams.get('email'))
  ...
  return (
    <>
      ...
      {/* registrar email */}
      {/* { emailx || session ? ( */}
      { !emailx && !emailxx && !session && !polo && !searchParams.get('verified') ? (
        ...
        ) : emailx && !emailxx && !session && polo ? (
          <Frente className="p-2 mt-2 text-small-regular sm:p-4 !bg-[#d7e5d9]">
            <div className={`w-full text-start text-[13px] text-[#020b1ddd] transition-[opacity] duration-300 sm:text-sm `}>
              <p className="mb-1 font-semibold text-sm sm:text-[15px]">{ name  /*sessionStorage.getItem('name') */ }</p>
              <p>Recibimos la consulta, te responderemos a la brevedad.</p>
              {/* { sessionStorage.getItem("emailxx") &&  */}
                <p className={`${emailxx && "hidden"}`}>{/*  */}
                  Por favor, revisá el correo electrónico <span className= "underline decoration-[#020b1d81] underline-offset-2 mx-1 ">{emailx}</span> y enviá la verificación.
                </p>
              {/* } */}
            </div>
          </Frente> 
          ...
          <Frente className="p-2 mt-2 text-small-regular sm:p-4 !bg-[#d7e5d9]">
            <div className={`w-full text-start text-[13px] text-[#020b1ddd] transition-[opacity] duration-300 sm:text-sm `}>
              <p className="mb-1 font-semibold text-sm sm:text-[15px]">{ session.user.name }</p>
              <p>Recibimos la consulta, te responderemos a la brevedad.</p>
            </div>
          </Frente>
        ) : (
          <Frente className="p-2 mt-4 text-small-regular sm:p-4 !bg-[#d7e5d9]">
            <div className={`w-full text-start text-[13px] text-[#020b1ddd] transition-[opacity] duration-300 sm:text-sm `}>
              {/* <p className="mb-1 font-semibold text-sm sm:text-[15px]">{ session?.user.name }</p> */}
              {/* Correo electrónico registrado: */}Te enviaremos la respuesta al email<span className= "underline decoration-[#020b1d81] underline-offset-2 mx-1 ">{session?.user.email} </span>
            </div>
          </Frente> 
        )
      }
        ...
        <div className="flex gap-4">
          ...
          { !emailx && !session && !polo ? (
            <form onSubmit={ files.length === 0 ? uploadToServer1 : uploadToServer2 } >
              <div className="group relative w-full flex justify-end items-center">
                ...
                >
                  <IconCambio
                    ...
                    className={`${isPending && "animate-spin"} mr-2 w-[22px] h-[22px] `}
                    // fill2="#fffc"
                    // fill="#ff00ff"
                  />
              ...
              <form onSubmit= { files.length === 0 ? uploadToServer1 : uploadToServer2 }  >
                <div className="group relative w-full flex justify-end items-center">
                  <span className={`opacity-0 invisible text-xs text-[#020b1d] absolute w-[170px] -bottom-[100%] bg-[#ffffff] pt-[3px] pb-[5px] pl-1.5 pr-3 rounded-lg duration-150 shadow-[0_20px_25px_-5px_rgb(0_0_0_/_0.2),_0_8px_10px_-6px_rgb(0_0_0_/_0.2),_0px_-5px_10px_#00000012] ${consulta ? "" : "group-hover:opacity-100"} sm:text-[13px] group-hover:visible`}><span className="text-base text-[#ff0000]">* </span>Completar requeridos</span>
                  <ButtonA
                    className={`h-8 !px-4 text-sm !justify-start disabled:!opacity-60`}
                    type="submit"
            ...
            <div className="flex justify-end items-center gap-4">
              {/* <Link href={"/dashboard/consultas"}>
                <ButtonA
                  className={`h-8 text-sm !justify-start ${session?.user.role === "member" && "hidden"}`}
                >
                  Ver Consultas
                </ButtonA>
              </Link> */}
